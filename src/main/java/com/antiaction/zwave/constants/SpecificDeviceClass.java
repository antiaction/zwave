...
public enum SpecificDeviceClass {
	...
	protected static final Map<Integer, Map<Integer, SpecificDeviceClass>> enumMap = new TreeMap<Integer, Map<Integer, SpecificDeviceClass>>();

	static {
		SpecificDeviceClass[] values = SpecificDeviceClass.values();
		SpecificDeviceClass value;
		Map<Integer, SpecificDeviceClass> sdcMap;
		for (int i=0; i<values.length; ++i) {
			value = values[i];
			sdcMap = enumMap.get(value.genericDeviceClass.id);
			if (sdcMap == null) {
				sdcMap = new TreeMap<Integer, SpecificDeviceClass>();
				enumMap.put(value.genericDeviceClass.id, sdcMap);
			}
			sdcMap.put(value.id, value);
		}
	}
	...
	public static Optional<SpecificDeviceClass> getType(GenericDeviceClass genericDeviceClass, int id) {
		SpecificDeviceClass sdc = null;
		Map<Integer, SpecificDeviceClass> sdcMap;
		if (genericDeviceClass != null) {
			sdcMap = enumMap.get(genericDeviceClass.id);
			sdc = sdcMap.get(id);
		}
		return Optional.ofNullable(sdc);
	}
