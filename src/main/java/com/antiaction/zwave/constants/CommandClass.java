...
public enum CommandClass {
	...
	protected static final Map<Integer, CommandClass> enumMap = new TreeMap<Integer, CommandClass>();

	static {
		CommandClass[] values = CommandClass.values();
		CommandClass value;
		for (int i=0; i<values.length; ++i) {
			value = values[i];
			enumMap.put(value.classCode, value);
		}
	}

	protected final int classCode;
	protected final String label;
	protected final int secondsRefreshInterval;
	protected final boolean supportsPolling;

	private CommandClass(int classCode, String label) {
		this(classCode, label, false, 0);
	}

	private CommandClass(int classCode, String label, boolean supportsPolling, int secondsRefreshInterval) {
	...
	public static Optional<CommandClass> getType(int classCode) {
		return Optional.ofNullable(enumMap.get(classCode));
	}
